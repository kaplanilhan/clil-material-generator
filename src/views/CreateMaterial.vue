    ...
    >
      <v-card>
        ...
        <v-card-text style="max-height: 70vh;">
           ...
           <div v-if="generatedMaterial?.metadata" class="text-caption text-grey mb-2">
               Generierungszeit: {{ generatedMaterial.metadata.generationTime.toFixed(1) }}s | Tokens: {{ generatedMaterial.metadata.tokensUsed }} | Modell: {{ generatedMaterial.metadata.model }}
           </div>
          ...
          <div v-else-if="generatedMaterial" v-html="generatedMaterial.content" class="preview-content"></div>
        ...
        <v-card-actions>
          ...
          <v-btn
            variant="text"
            @click="previewDialog = false; currentStep = 4;"
            :disabled="generating"
          >
...
const rules = computed(() => {
  return {
    type: { required: helpers.withMessage('Bitte wählen Sie einen Material-Typ.', required) },
    subject: { required: helpers.withMessage('Bitte wählen Sie ein Fach.', required) },
    topic: { 
...
// Material types
const materialTypes = [
  { id: 'worksheet', title: 'Arbeitsblatt', description: 'Übungsblatt mit Aufgaben & Lösungen', color: 'green', icon: 'mdi-file-document-outline' },
  { id: 'quiz', title: 'Quiz', description: 'Multiple-Choice oder Lückentext', color: 'blue', icon: 'mdi-comment-question-outline' },
  { id: 'glossary', title: 'Glossar', description: 'Fachwörterbuch mit Definitionen', color: 'orange', icon: 'mdi-book-open-page-variant-outline' },
  { id: 'presentation', title: 'Präsentation', description: 'Folien für den Unterricht', color: 'purple', icon: 'mdi-presentation' },
  { id: 'graphic', title: 'Grafik', description: 'Diagramme und Visualisierungen', color: 'red', icon: 'mdi-chart-bar' },
  { id: 'video', title: 'Video-Skript', description: 'Drehbuch für Lehrvideos', color: 'grey', icon: 'mdi-video' }
];
...
const hasStepError = (index) => {
  if (index === 0 && v$.value.type.$invalid && v$.value.type.$dirty) return true;
  if (index === 1 && (v$.value.subject.$invalid || v$.value.topic.$invalid) && (v$.value.subject.$dirty || v$.value.topic.$dirty)) return true;
  return false;
};
...
const canProceed = computed(() => {
  ...
  if (currentStep.value === 2) return !v$.value.subject.$invalid && !v$.value.topic.$invalid;
  return true; 
});
...
const generateMaterialAction = async () => {
  ...
  try {
    const response = await geminiApi.generateMaterial(form.value.type, {
...
const saveMaterial = async () => {
  ...
  try {
    const newMaterialData = {
      title: generatedMaterial.value.title || `${form.value.subject}: ${form.value.topic}`,
